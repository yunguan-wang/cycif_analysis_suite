...
pooled_expr_data = pd.read_csv(
    '_example_input/sample_exprdata.csv', index_col=0)
pooled_metadata = pd.read_csv(
    '_example_input/sample_metadata.csv', index_col=0)
if not os.path.exists('_example_output'):
    os.makedirs('_example_output')

control_name = 'ctrl'
output_prefix = 'T0_ctrl as control'
pwa = per_well_analysis(pooled_expr_data, pooled_metadata, batch_col='replicate',
                        drug_col='ligand', dose_col='dose', time_col='time', control_name=control_name,
                        output_path='_example_output', output_prefix=output_prefix)
