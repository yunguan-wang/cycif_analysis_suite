...
from common_apis import differential_analysis, make_complex_heatmap, two_way_hc_ordering,check_numeric, channel_histograms
...
class per_well_analysis():
    ...
    def descriptive_analysis(self, 
        ...
        if channel_dist_plot:
            fig_name = os.path.join(self.output_path, ' '.join([self.output_prefix,fig_name]))
            channel_histograms(self.expr_data,save_fig_filename = fig_name)
        # Evaluate total variance.
        total_variance = self.expr_data.groupby([drug_group,dose_group,time_group]).std().sum(axis = 1)
        if return_variance:
            return total_variance
        else:
            total_variance_fn = os.path.join(self.output_path, ' '.join([self.output_prefix, 'log2 transformed data total variance by condition.csv']))
    ...
    def make_contrast(self, batch_wise = False, test_condition_groupby = None, control_name = None, control_col_idx = 0):
        ...
        if test_condition_groupby is None:
            test_condition_groupby = [self.drug_col,self.dose_col,self.time_col]
        ...
        if control_name!=self.control_name:
            print('Control sample changed to {}:{} from {}:{}!'.format(control_col, control_name, self.control_col, self.control_name))

        # batch-wise operation, each batch works with only samples of current batch. 
        for batch in list_batches:
            batch_contrasts = []
            if batch=='single_batch':
                current_batch_samples = self.metadata.index
            else:
                current_batch_samples = self.metadata[self.metadata[self.batch_col]==batch].index
            current_batch = self.metadata.loc[current_batch_samples]
            current_batch_agg = current_batch.groupby(test_condition_groupby).agg(lambda x: ','.join(x.index)).iloc[:,0]
            ...
            # if no controls were found, throw an exception and return some information on why it is so.
            if _status==0:
                print('Control condition not found!')
                print('This is all conditions in batch {}:{}:'.format(self.batch_col, str(batch)))
            ...
            for idx in current_batch_agg.index:
                if idx[control_col_idx]==control_name:
                    ...
                    batch_contrasts.append(('_'.join([str(x) for x in idx]),current_batch_agg.loc[idx],control_cells_metadata,control_cells,batch))
    ...
    def contrast_based_distance(self):
        ...
        for contrast in self.list_all_contrasts:
            ...
            control_vector = control_expr.median().reshape(1, -1)
            ...
            dist_vector = dist_vector[(dist_vector>low)&(dist_vector<high)]
    ...
    def plot_dist_violinplot(self, facetgrid_col, x_group, facetgrid_row=None):
        ...
        '''
        save_fig_filename = os.path.join(self.output_path, ' '.join(['Drug_control distance over',x_group, 'per', facetgrid_col]) + '.png')
        sns.set(font_scale=1.5)
        order = sorted(self.metadata[x_group].unique())
        g = sns.FacetGrid(data=self.df_dist,col=facetgrid_col,row=facetgrid_row, palette='Blues',sharey=True, sharex=True,height=6)
...
# plot_dist_violinplot(df_dist,x_axis_col='Time',save_fig_filename=output_prefix+' Treatment to control distance.png')
    ...
    def contrast_based_differential_analysis(self,save_report = True):
        ...
        for batch in self.list_all_contrasts:
            ...
            batch_id = batch[4]
            batch_de_report = differential_analysis(self.expr_data.loc[test_samples],self.expr_data.loc[control_samples])
        ...
        if save_report is True:
            output_fname = os.path.join(self.output_path,self.output_prefix,'logFC report.xlsx')
    ...
    def make_fc_heamtap_matrix(self, condition_by = None, output_fname = None):
        ...
        df_fc['condition'] = df_fc[condition_by].astype(str).apply(lambda x: '_'.join(x),axis = 1)
        
        if df_fc.groupby('condition').logFC.count()[0]!=df_fc.index.unique().shape[0]:
            no_replicates = (df_fc.groupby('condition').logFC.count()[0]) / (df_fc.index.unique().shape[0])
            print('There seem to be multiple values ({}) for the same condition. Consider modify the "condition_by" paramerter!"'.format(str(no_replicates)))
            return
        ...
        try:
            df_fc.loc[self.time_col][2:] = df_fc.loc[self.time_col][2:].astype(int).apply(lambda x: "{0:0=2d}h".format(x))
        except:
            pass
        if output_fname is not None:
            output_name = os.path.join(self.output_path, self.output_prefix + 'logFC heatmap.csv')
    ...
    def make_heatmaps(self, sorting = '2D_HC'):
        ...
        heatmap_cmap = sns.palettes.diverging_palette(240,0,s=99,as_cmap=True)            
        df_data = self.fc_heatmap.iloc[n_metacols:,2:].astype(float)
        output_fname = os.path.join(self.output_path, ' '.join([self.output_prefix, sorting, 'logFC heatmap.png']))
        ...
        if sorting=='2D_HC':
            ...
            cols_order = self.fc_heatmap.iloc[:,2:].sort_values(self.fc_heatmap_col_meta,axis = 1).columns
            row_order = self.fc_heatmap.iloc[n_metacols:,:].sort_values(['Location','Marker']).index
    ...
    def make_swarm_plot(self, color_by = None, x_group = None):
        ...
        output_fname = os.path.join(self.output_path,' '.join([self.output_prefix,'swarm logFC plot by',x_group, '.png']))
        df_fc = self.report.copy()
        # Assumes the first two '_' separated elements of index are location and channel name
        df_fc['Ch'] = ['_'.join(x.split('_')[:2]) for x in df_fc.index]
        df_fc.sort_values(x_group,inplace = True)
        g = sns.FacetGrid(data=df_fc,col = 'Ch',hue=color_by,col_wrap=5,height=5,aspect = 2)
    ...
    def channel_dist_plot(self, fc_threshold = 0.6, organize_by = None):
        ...
        df_fc = abs(df_fc.groupby(self.contrast_by + ['ch']).logFC.mean()).reset_index()
        #TODO: implement col sep by dose option
        df_fc.sort_values(organize_by,inplace=True)
        df_fc['ls'] = '-'
        df_fc.loc[df_fc.logFC<fc_threshold,'ls'] = ':'
        df_fc = df_fc.set_index('ch')

        # Reformate expr data into long format
        overall_data = self.expr_data.reset_index()
        ...
        overall_data = overall_data.merge(self.metadata[['ligand','time']], left_index=True,right_index=True,sort=False,how='left')
        overall_data.sort_values(organize_by,inplace=True)
        overall_data.set_index('Channel',inplace=True)

        # Plotting
        sns.set(font_scale=1.5)
        for channel in df_fc.index.unique():
            output_fname = ' '.join([self.output_prefix, channel.replace('/', '+'), 'logFC distribution overtime.png'])
            output_fname = os.path.join(self.output_path,'misc',output_fname)
            channel_logfc = df_fc.loc[channel]
            channel_data = overall_data.loc[channel]
            ls = channel_logfc.groupby(organize_by).ls.first()
            ...
            for sub_plot in g.axes.flatten():
                ...
                for line in sub_plot.lines:
                    ...
                    if ligand == self.control_name:
                        continue
                    line.set_linestyle(ls[(time,ligand)])
            plt.savefig(output_fname)
            plt.close()
        sns.set(font_scale=1)