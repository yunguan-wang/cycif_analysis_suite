...
def umap_parameter_opt(df_data, nn, md, sample_size = None):
    ...
    if sample_size is not None:
        random_idx = np.random.choice(df_data.index,10000, False, verbose = False)
    ...
    for n_neighbors in nn:
        for min_dist in md:
            ...
            if verbose:
                print(batch)
            umap = UMAP(n_neighbors = n_neighbors,n_components=2,min_dist=min_dist, random_state=50)
...
def make_fc_time_plots(df_fc,control_name = 'DMSO'):
    ...
    for drug in df_fc.DrugName.unique():
        ...
        g = sns.FacetGrid(sub_df_fc,col_wrap=10,col='index',hue='Time',hue_order=time_order, height=5)
...
def two_way_hc_ordering(data):
    ...
    col_order = sch.dendrogram(lm_t,no_plot = True, distance_sort = 'ascending')['leaves']
...
def calculate_cluster_composition(df_metadata):
    df_metadata['condition'] = df_metadata.DrugName + '_' + df_metadata.Conc.astype(str)
    df_freq = df_metadata.groupby(['cluster','condition']).count().iloc[:,0]
    df_freq = df_freq.groupby('cluster').apply(lambda x: x/x.sum()).reset_index()
    df_freq = df_freq.pivot(index='cluster', columns='condition', values='well')
...
def make_complex_heatmap(df_data, heatmap_cmap = 'coolwarm',
    ...
    if n_row*n_col>1:
        ...
        axes = axes.ravel()
        main_fig = sns.heatmap(df_data, vmax=vmax, vmin=vmin,ax = axes[0], cbar = False, cmap = heatmap_cmap,robust = True)
    else:
        main_fig = sns.heatmap(df_data, vmax=vmax, vmin=vmin, cbar = False,cmap = heatmap_cmap, robust = True)
    # Make the main heatmap as the first subplot    
    main_fig_axes = fig.add_axes([0.13,0.95,0.7,0.05])
    main_fig_cb = plt.colorbar(main_fig.get_children()[0], orientation = 'horizontal', cax=main_fig_axes)
    main_fig_cb.ax.set_title("Heatmap",position = (1.06,0.1),fontsize = 16)
    main_fig.set_xticks([])
    main_fig.set_yticks([])
    main_fig.set_ylabel('logFC change compared with corresponding DMSO', fontsize = 14)
    # Iterate through each metadata dataframe and start ploting the color bar and heatmaps row-wise or column-wise
    for metadata, base_anchor,anchor_offset_location in zip([row_metadata,col_metadata],[row_colorbar_anchor,col_colorbar_anchor],[0,1]):
        ...
        for col in metadata.columns:
            ...
            # Handling descrete heatmap sidebar values, which are factorized.
                ...
                metadata_vector = pd.DataFrame(metadata_vector.factorize()[0],columns = [col])
                cmap = sns.color_palette("cubehelix_r",levels.shape[0])
                cb_type = 'discreet'

            # Calculate the axes index and location of the "legend" of the sidebar, which are actually colorbar objects.
            ...
            anchor = base_anchor
            anchor[anchor_offset_location] = anchor[anchor_offset_location] + offset
            colorbar_ax = fig.add_axes(anchor)
            g = sns.heatmap(metadata_vector, ax = ax, cbar = False,xticklabels=False, yticklabels=False, cmap = cmap, vmax = metadata_vector.values.max()+1)
#             g.set_title(col)
            if cb_type != 'continuous':
                cb = plt.colorbar(g.get_children()[0], orientation = cbar_label_orientation, cax=colorbar_ax)
                # Make correct ticks and tick labels, need to offset the lenth to fix the miss-by-one problem.
    ...
    valid_axes_id  = [x for x in range(n_col)]+[x*n_row for x in range(n_col)]
